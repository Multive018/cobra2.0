...
interface StockRow {
    ...
    'Intake Date'?: string | number | Date;
    [key: string]: any; // Allow other columns
}

// Helper: Format Date
// Excel/CSV can return dates as Objects, serial numbers, or strings.
function formatDateForMySQL(dateVal: string | number | Date): string | null {
    ...
    if (dateVal instanceof Date) {
        ...
        // Note: If CSV date is DD/MM/YYYY, ensure system locale supports it or parse manually
...
export async function update_raw_material__intake_date(file: File) {
    console.time("Total Execution Time");
    
    let rawData: StockRow[] = [];

    // --- STEP 1: Read File ---
    try {
        const buffer = await file.arrayBuffer();
        // type: 'array' works for both Excel binaries and CSV text
        // cellDates: true attempts to parse date strings into JS Date objects
        ...
        rawData = XLSX.utils.sheet_to_json<StockRow>(worksheet, { range });
        console.log(`File parsed. Found ${rawData.length} rows.`);
    ...
    for (let i = 0; i < rawData.length; i++) {
        ...
        // Normalize string values for comparison (safely handle numbers/nulls)
        const allocation = String(row['Position Strategy Allocation'] || '').trim();
        const warehouse = String(row['Warehouse'] || '').trim();
        const growerMarks = String(row['Grower Marks'] || '').trim();
        const blocked = String(row['Blocked'] || '').trim();

        // --- FILTER LOGIC ---
        let rejectionReason = "";

        if (!allocation.startsWith('PRE')) {
            rejectionReason = `Allocation (${allocation}) != PRE`;
        } else if (warehouse !== 'KAHAWA BORA WAREHOUSE') {
            rejectionReason = `Warehouse (${warehouse}) != KAHAWA BORA WAREHOUSE`;
        } else if (!growerMarks) {
            rejectionReason = `Grower Marks is empty`;
        } else if (blocked !== 'No') {
            rejectionReason = `Blocked (${blocked}) != No`;
        }

        if (rejectionReason === "") {
            results.push(row);
        } else {
            // Debug: Only show first 5 rejections
            if (i < 5) console.log(`Skipped Row ${i + 2}: ${rejectionReason}`);
        }
    }

    console.log(`Filtering Complete. Found ${results.length} valid records to process.`);

    if (results.length === 0) {
        console.warn("STOPPING: No matching records found after filtering.");
        console.timeEnd("Total Execution Time");
        return;
    }

    try {
        // --- STEP 3: Batch Existence Check ---
        const candidateBatchNumbers = results.map(r => String(r['Batch No.']));

        // Check which of these batches already exist in the DB
        const existingRows = await query<RowDataPacket[]>({
            query: `SELECT batch_number FROM raw_batches_intake_dates WHERE batch_number IN (?)`,
            values: [candidateBatchNumbers]
        });

        const existingBatches = new Set<string>();
        if (existingRows) {
            existingRows.forEach((row: any) => existingBatches.add(row.batch_number));
        }

        // --- STEP 4: Prepare Final Insert List ---
        const valuesToInsert: [string, string][] = [];
        const seenInFile = new Set<string>();

        for (const row of results) {
            const batchNum = String(row['Batch No.']);
            
            if (existingBatches.has(batchNum)) continue;
            if (seenInFile.has(batchNum)) continue;

            // Type assertion used safely because logic implies it exists if filtered correctly
            const dateIn = formatDateForMySQL(row['Intake Date']!); 
            
            if (!dateIn) {
                console.warn(`Skipping batch ${batchNum}: Invalid date format ${row['Intake Date']}`);
                continue;
            }

            valuesToInsert.push([dateIn, batchNum]);
            seenInFile.add(batchNum);
        }

        // --- STEP 5: Bulk Insert ---
        if (valuesToInsert.length > 0) {
            console.log(`Inserting ${valuesToInsert.length} new records...`);
            
            const insertResult = await query<ResultSetHeader>({
                query: `INSERT INTO raw_batches_intake_dates (date, batch_number) VALUES ?`,
                values: [valuesToInsert]
            });

            if (insertResult) {
                console.log(`SUCCESS: Inserted ${insertResult.affectedRows} new records.`);
            }
        } else {
            console.log("No new records to insert. All valid batches already exist.");
        }
...
export async function update_post_process_intake_dates() {
    console.time("Processing Time");

    // 1. GET DATA: Find all processes where at least one output has a NULL date_in
    // We fetch BOTH inputs and outputs for these processes in one go.
    const sqlGetPending = `
        SELECT id, process_id, date_in, batch_number, input_qty, output_qty 
        FROM daily_strategy_processing 
        WHERE process_id IN (
            SELECT DISTINCT process_id 
            FROM daily_strategy_processing 
            WHERE output_qty > 0 AND date_in IS NULL
        )
    `;

    const allRows = await query<RowDataPacket[] & ProcessRow[]>({ query: sqlGetPending });

    if (!allRows || allRows.length === 0) {
        console.log("No pending output dates to process.");
        console.timeEnd("Processing Time");
        return;
    }

    // 2. GROUPING: Group by process_id in memory
    const processMap = new Map<number, { inputs: ProcessRow[], outputs: ProcessRow[] }>();

    for (const row of allRows) {
        if (!processMap.has(row.process_id)) {
            processMap.set(row.process_id, { inputs: [], outputs: [] });
        }
        const group = processMap.get(row.process_id)!;
        
        // Categorize based on qty (as per your instructions)
        if (Number(row.input_qty) > 0) group.inputs.push(row);
        else if (Number(row.output_qty) > 0) group.outputs.push(row);
    }

    // 3. IDENTIFY MISSING INPUT DATES
    const batchesNeedingDates = new Set<string>();
    
    processMap.forEach((group) => {
        group.inputs.forEach(input => {
            if (!input.date_in) {
                batchesNeedingDates.add(input.batch_number);
            }
        });
    });

    const missingBatchList = Array.from(batchesNeedingDates);
    const resolvedDates = new Map<string, Date>();

    // 4. BULK LOOKUP (Optimized)
    if (missingBatchList.length > 0) {
        console.log(`Resolving dates for ${missingBatchList.length} unique input batches...`);

        // A. Look in daily_strategy_processing (History)
        // We get the most recent non-null date for these batches
        const sqlHistory = `
            SELECT batch_number, MAX(date_in) as date_in 
            FROM daily_strategy_processing 
            WHERE batch_number IN (?) AND date_in IS NOT NULL 
            GROUP BY batch_number
        `;
        const historyMatches = await query<RowDataPacket[] & DateLookup[]>({ 
            query: sqlHistory, 
            values: [missingBatchList] 
        });

        if (historyMatches) {
            historyMatches.forEach(r => resolvedDates.set(r.batch_number, new Date(r.date_in)));
        }

        // B. Look in raw_batches_intake_dates (Raw Data)
        // Only look for ones we haven't found yet
        const stillMissing = missingBatchList.filter(b => !resolvedDates.has(b));
        
        if (stillMissing.length > 0) {
            const sqlRaw = `
                SELECT batch_number, date 
                FROM raw_batches_intake_dates 
                WHERE batch_number IN (?)
            `;
            const rawMatches = await query<RowDataPacket[]>({ 
                query: sqlRaw, 
                values: [stillMissing] 
            });

            if (rawMatches) {
                rawMatches.forEach(r => resolvedDates.set(r.batch_number, new Date(r.date)));
            }
        }
    }

    // 5. CALCULATE UPDATES
    const updatesToExecute: { id: number, date_in: string }[] = [];
    const today = new Date(); // Reference for "today"

    for (const [processId, group] of processMap) {
        let allInputsHaveDates = true;
        let totalWeightedDays = 0;
        let totalInputQty = 0;

        // A. Update Input Batches locally and queue DB updates
        for (const input of group.inputs) {
            if (!input.date_in) {
                const foundDate = resolvedDates.get(input.batch_number);
                if (foundDate) {
                    input.date_in = foundDate; // Update in memory object for calculation below
                    updatesToExecute.push({ 
                        id: input.id, 
                        date_in: foundDate.toISOString().split('T')[0] 
                    });
                } else {
                    allInputsHaveDates = false;
                }
            }
        }

        // Skip this process if we can't fully resolve inputs
        if (!allInputsHaveDates || group.inputs.length === 0) continue;

        // B. Calculate Output Date
        let calculatedOutputDate: Date;

        if (group.inputs.length === 1) {
            // Direct copy
            calculatedOutputDate = new Date(group.inputs[0].date_in!);
        } else {
            // Weighted Average
            for (const input of group.inputs) {
                const inputDate = new Date(input.date_in!);
                const qty = Number(input.input_qty);

                // Calculate days in stock: Today - InputDate
                const timeDiff = today.getTime() - inputDate.getTime();
                const daysInStock = timeDiff / (1000 * 3600 * 24);

                totalWeightedDays += (daysInStock * qty);
                totalInputQty += qty;
            }

            if (totalInputQty === 0) continue; // Prevent div by zero

            const weightedAvgDays = totalWeightedDays / totalInputQty;
            
            // Result = Today - WeightedAvgDays
            const resultTime = today.getTime() - (weightedAvgDays * (1000 * 3600 * 24));
            calculatedOutputDate = new Date(resultTime);
        }

        // C. Queue updates for ALL outputs in this process
        const dateString = calculatedOutputDate.toISOString().split('T')[0];
    ...
    if (updatesToExecute.length > 0) {
        console.log(`Executing updates for ${updatesToExecute.length} records...`);
        
        const connection = await pool!.getConnection();
        try {
            await connection.beginTransaction();

            // Constructing a bulk update query using CASE is most efficient for large sets,
            // but for simplicity and safety against packet limits, we loop prepared statements.
            // Since we are reusing the connection, this is still fast.
            for (const update of updatesToExecute) {
                await connection.query(
                    `UPDATE daily_strategy_processing SET date_in = ? WHERE id = ?`,
                    [update.date_in, update.id]
                );
            }

            await connection.commit();
            console.log("Successfully updated intake dates.");
        } catch (error) {
        ...
        console.log("No updates could be calculated based on available data.");
    }
    ...
    for (const [stackType, batches] of Object.entries(groupedStacks)) {
        ...
        for (const batch of batches) {
            ...
            if (!batchNumber || !Number.isFinite(quantity) || quantity <= 0) {
                skipReason = 'Invalid Batch No. or Quantity in input file.';
...
export async function analysis_vs_actual(process_id: number) {
    console.time("Analysis Calculation");

    // 1. Fetch Process Data
    const sqlProcess = `
        SELECT id, batch_number, input_qty, output_qty, analysis_id 
        FROM daily_strategy_processing 
        WHERE process_id = ?
    `;
    const rows = await query<(ProcessRow & RowDataPacket)[]>({ 
        query: sqlProcess, 
        values: [process_id] 
    });
    ...
    const inputs: ProcessRow[] = [];
    const outputs: ProcessRow[] = [];
    const relevantAnalysisIds = new Set<number>();

    for (const row of rows) {
        const inQty = Number(row.input_qty);
        const outQty = Number(row.output_qty);
        const aId = row.analysis_id ? Number(row.analysis_id) : null;

        if (inQty > 0) {
            // Strict Validation for Inputs
            if (!aId) {
                console.log(`Aborting: Input batch ${row.batch_number} has no analysis_id.`);
                return;
            }
            inputs.push(row);
            relevantAnalysisIds.add(aId);
        } else if (outQty > 0) {
            outputs.push(row);
            if (aId) relevantAnalysisIds.add(aId);
        }
    }

    if (inputs.length === 0) return; 

    // 3. Bulk Fetch Analysis Data
    const analysisIdsArr = Array.from(relevantAnalysisIds);
    if (analysisIdsArr.length === 0) return;

    const [analysisRows, screenRows, classRows] = await Promise.all([
        query<(BatchAnalysisRow & RowDataPacket)[]>({
            query: `SELECT * FROM batch_analysis WHERE id IN (?)`,
            values: [analysisIdsArr]
        }),
        query<(ScreenSizeRow & RowDataPacket)[]>({
            query: `SELECT * FROM screensize_breakdown WHERE analysis_id IN (?)`,
            values: [analysisIdsArr]
        }),
        query<(ClassScreenRow & RowDataPacket)[]>({
            query: `SELECT * FROM class_by_screensize WHERE analysis_id IN (?)`,
            values: [analysisIdsArr]
        })
    ]);

    // 4. Index Data for O(1) Lookup
    const analysisMap = new Map<number, BatchAnalysisRow>();
    const screenMap = new Map<number, ScreenSizeRow[]>();
    const classMap = new Map<number, ClassScreenRow[]>();

    analysisRows?.forEach(r => analysisMap.set(r.id, r));
    
    screenRows?.forEach(r => {
        if (!screenMap.has(r.analysis_id)) screenMap.set(r.analysis_id, []);
        screenMap.get(r.analysis_id)!.push(r);
    });

    classRows?.forEach(r => {
        if (!classMap.has(r.analysis_id)) classMap.set(r.analysis_id, []);
        classMap.get(r.analysis_id)!.push(r);
    });

    // Helper: Aggregation Logic
    const aggregateBatches = (batches: ProcessRow[], isInput: boolean) => {
        let totalWeight = 0;
        
        // Accumulators
        const mainAnalysisAcc = {
            profile_print_score: 0, sca_defect_count: 0, primary_defects_percentage: 0,
            secondary_defects_percentage: 0, forein_matter_percentage: 0,
            grade_aa_percentage: 0, grade_ab_percentage: 0, grade_abc_percentage: 0,
            grade_grinder_percentage: 0
        };
        const screenAcc = new Map<number, number>(); 
        const classAcc = new Map<string, number>(); 

        // Loop Batches
        for (const batch of batches) {
            const weight = isInput ? Number(batch.input_qty) : Number(batch.output_qty);
            const aId = Number(batch.analysis_id);
            const analysis = analysisMap.get(aId);

            if (!analysis) continue; 

            totalWeight += weight;

            // 1. Main Analysis Weighted Sum
            (Object.keys(mainAnalysisAcc) as Array<keyof typeof mainAnalysisAcc>).forEach(key => {
                mainAnalysisAcc[key] += (Number(analysis[key] || 0) * weight);
            });

            // 2. Screensize Weighted Sum
            const screens = screenMap.get(aId) || [];
            screens.forEach(s => {
                const current = screenAcc.get(s.screen_size) || 0;
                screenAcc.set(s.screen_size, current + (Number(s.percentage) * weight));
            });

            // 3. Class By Screen Weighted Sum
            const classes = classMap.get(aId) || [];
            classes.forEach(c => {
                const key = `${c.screen_size}|${c.class}`;
                const current = classAcc.get(key) || 0;
                classAcc.set(key, current + (Number(c.percentage) * weight));
            });
        }
        ...
        (Object.keys(mainAnalysisAcc) as Array<keyof typeof mainAnalysisAcc>).forEach(key => {
            finalAnalysis[key] = Number((mainAnalysisAcc[key] / totalWeight).toFixed(3));
        });

        const finalScreens: any[] = [];
        screenAcc.forEach((val, key) => {
            finalScreens.push({ screen_size: key, percentage: Number((val / totalWeight).toFixed(3)) });
        });

        const finalClasses: any[] = [];
        classAcc.forEach((val, key) => {
            const [size, cls] = key.split('|');
            finalClasses.push({ 
                screen_size: Number(size), 
                class: cls, 
                percentage: Number((val / totalWeight).toFixed(3)) 
            });
        });

        // RETURN STRUCTURE with Quantities
        const batchList = batches.map(b => ({
            batch_number: b.batch_number,
            qty: isInput ? Number(b.input_qty) : Number(b.output_qty)
        }));

        return {
            batches: batchList,
            aggregated_analysis: finalAnalysis,
            aggregated_screensize_breakdown: finalScreens.sort((a,b) => b.screen_size - a.screen_size),
            aggregated_class_by_screensize: finalClasses.sort((a,b) => b.screen_size - a.screen_size)
        };
    };

    // 5. Run Aggregation
    const inputResult = aggregateBatches(inputs, true);
    const outputResult = aggregateBatches(outputs, false);

    console.timeEnd("Analysis Calculation");

    // Construct fallback for outputs if no analysis found (still show batches)
    const fallbackOutputs = {
        batches: outputs.map(b => ({
            batch_number: b.batch_number,
            qty: Number(b.output_qty)
        })),
        message: "No analysis data available for outputs"
    };
