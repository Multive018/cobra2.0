...
// export async function processStiFile(targetDate: Date, 
//   stiFile: File | null, 
//   summary_id: number,
...
export async function processStiFile(
  ...
  let allRows: any[];
  try {
    if (!stiFile) throw new Error("STI File is null or empty.");
    const buffer = await stiFile.arrayBuffer();
    const workbook = XLSX.read(buffer, { type: 'buffer', cellDates: true });
    const sheetName = workbook.SheetNames[0];
    const worksheet = workbook.Sheets[sheetName];

    if (!worksheet) throw new Error("STI file seems to be empty or invalid.");
    
    allRows = XLSX.utils.sheet_to_json<any>(worksheet, { range: 1 });
    console.log(`[Step 1] Rows read: ${allRows.length}`);
  } catch (error) {
    console.error("[Step 1] Failed to parse STI Excel:", error);
    throw error;
  }
  
  // --- 1.5 & 1.6: Build Strategy Map and Augment Rows (Combined) ---
  const stockStrategyMap = new Map<string, string>();
  if (currentStockFile) {
    try {
      const stockBuffer = await currentStockFile.arrayBuffer();
    ...
    } catch (e) { console.error('[Step 1.5] Stock file error:', e); }
  }

  // Optimization: One pass for Strategy + Date Correction (Noon Fix)
  allRows.forEach((row: any) => {
    const batchNum = row['Batch No.']?.toString().toUpperCase();
    row['Strategy'] = batchNum ? (stockStrategyMap.get(batchNum) || 'UNDEFINED') : 'UNDEFINED';
  ...
  // --- 2. Sync Existing Pending Batches ---
  let affectedStiIdsFromSync: Set<number> = new Set();
  try {
    // @ts-ignore - Assuming these exist in the wider scope/file
    affectedStiIdsFromSync = await syncPendingBatches(allRows);
  } catch (error) {
    console.error("[Step 2] Error during sync:", error);
  }

  // --- 3. Get latest arrival_date from DB & Filter Rows ---
  let dbLatestTime: number;
  try {
    const dbResult = await query({
      query: `SELECT arrival_date FROM instructed_batches ORDER BY arrival_date DESC LIMIT 1`
    });

    const rows = dbResult as any[];
    const latestDate = rows && rows.length > 0 ? rows[0].arrival_date : null;
    
    // OPTIMIZATION: Sentinel Fallback (-1).
    // If DB is empty, latestDate is null -> dbLatestTime = -1.
    // The filter check (timestamp > -1) will be TRUE for all valid Excel dates.
    dbLatestTime = latestDate ? new Date(latestDate).getTime() : -1;
    console.log(`[Step 3] Latest Arrival Date: ${latestDate || 'NONE (Processing all rows)'}`);
  } catch (error: any) {
    console.error(`[Step 3] DB Date Fetch failed: ${error.message}`);
    throw error;
  }
  ...
  const dateFilteredRows = allRows.filter(row => {
    ...
    return (
      ...
      transactionDate.getTime() > dbLatestTime
    );
  });
  ...
  if (dateFilteredRows.length === 0) {
    // @ts-ignore
    await updateStiHeaderStatuses(affectedStiIdsFromSync);
    console.log("--- processStiFile END (No new data) ---");
    return 0; 
  }

  // --- 4. Calculate Total Delivered Qty ---
  const total_delivered_qty = dateFilteredRows.reduce((acc, row) => acc + parseSafeFloat(row['Qty._2']), 0);

  // --- 5. Aggregate STI Data ---
  const uniqueStiStrings = [...new Set(dateFilteredRows.map(row => row['Number']?.toString()).filter(Boolean))] as string[];
  const aggregatedStis: any[] = [];

  for (const stiNum of uniqueStiStrings) {
    const matchingRows = allRows.filter(row => row['Number']?.toString() === stiNum);
    if (matchingRows.length === 0) continue;

    aggregatedStis.push({
      sti_number: stiNum,
      instructed_date: matchingRows[0]['Date'] as unknown as Date,
      instructed_qty: matchingRows.reduce((acc, r) => acc + parseSafeFloat(r['Qty.']), 0),
      delivered_qty: matchingRows.reduce((acc, r) => acc + parseSafeFloat(r['Qty._2']), 0),
      loss_gain: matchingRows.reduce((acc, r) => acc + parseSafeFloat(r['Qty._3']), 0),
    });
  }
  ...
  for (const sti of aggregatedStis) {
    const upsertQuery = `
      ...
      VALUES (?, ?, ?, ?, ?, ?)
      ON DUPLICATE KEY UPDATE
        summary_id = VALUES(summary_id), instructed_date = VALUES(instructed_date),
        instructed_qty = VALUES(instructed_qty), delivered_qty = VALUES(delivered_qty), loss_gain = VALUES(loss_gain)
    `;
    await query({
      query: upsertQuery,
      values: [summary_id, sti.sti_number, formatDateForMySQL(sti.instructed_date), sti.instructed_qty, sti.delivered_qty, sti.loss_gain]
    });
  }

  // --- 7. Map STI Numbers to IDs ---
  const stiRows = await query<any[]>({
    query: `SELECT id, sti_number FROM stock_transfer_instructions WHERE sti_number IN (?)`,
    values: [uniqueStiStrings]
  });
  ...
  // --- 8 & 9. Filter New Batches ---
  const newBatchesToInsert: any[] = [];
  for (const row of dateFilteredRows) {
    const batch_number = row['Batch No.']?.toString();
    const transaction_number = row['Transaction No.']?.toString();
    const arrival_date = row['Transaction Date_1'] as unknown as Date;
    const sti_num = row['Number']?.toString();
    const sti_id = sti_num ? stiNumberToIdMap.get(sti_num) : null;

    if (!batch_number || !transaction_number || !arrival_date || !sti_id) continue;

    const existing = await query<any[]>({
      query: `SELECT id FROM instructed_batches WHERE batch_number = ? AND transaction_number = ? AND arrival_date = ?`,
      values: [batch_number, transaction_number, formatDateForMySQL(arrival_date)]
    });
    if (existing && existing.length > 0) continue;

    const del_qty = parseSafeFloat(row['Qty._2']);
    const status_raw = row['Stock Transfer Status'];
    let status = status_raw === 'Completed' ? 'completed' : (del_qty > 0 ? 'partially_delivered' : 'fully_pending');

    newBatchesToInsert.push([
      summary_id, sti_id, row['Item Name'] || 'UNKNOWN', row['Strategy'] || 'UNDEFINED',
      parseSafeFloat(row['Qty.']), del_qty, parseSafeFloat(row['Qty._5']),
      parseSafeFloat(row['Qty._3']), status, row['From WareHouse - Zone'] || 'UNDEFINED',
      row['Storage Due Date'] instanceof Date ? formatDateForMySQL(row['Storage Due Date'] as Date) : null,
      formatDateForMySQL(arrival_date), transaction_number, batch_number
    ]);
  }
  ...
  if (newBatchesToInsert.length > 0) {
    await query({
      query: `INSERT INTO instructed_batches (summary_id, sti_id, grade, strategy, instructed_qty, delivered_qty, balance_to_transfer, loss_gain_qty, status, from_location, due_date, arrival_date, transaction_number, batch_number) VALUES ?`,
      values: [newBatchesToInsert]
    });
  }

  // --- 11 & 12. Finalize ---
  // @ts-ignore
  await updateStiHeaderStatuses(new Set([...affectedStiIdsFromSync, ...affectedStiIdsFromUpsert]));
  return total_delivered_qty;
}
...
export async function processOutbounds(
  ...
  try {
    ...
    // SENTINEL FALLBACK: If no row found, -1 allows all rows (> -1)
    dbLatestTime = latestDateStr ? new Date(latestDateStr).getTime() : -1;
    console.log(`[Step 1] Latest DB Date: ${latestDateStr || 'NONE (Processing all rows)'}`);
  } catch (error: any) {
    console.error(`[Step 1] DB Date Fetch failed: ${error.message}`);
    throw error;
  }
  ...
  const dateFilteredRows = allRows.filter((row: any) => {
    const dcDate = row['DC Date'] as unknown as Date;
    return (dcDate instanceof Date && !isNaN(dcDate.getTime()) && dcDate.getTime() > dbLatestTime);
  });

  if (dateFilteredRows.length === 0) return { totalOutbound: 0, groupedData: [] };
 ...
 * processAdjustments
 * Optimizations:
 * 1. SQL Top-1: Retrieves only the latest date from DB (O(log N) with index).
 * 2. Sentinel Fallback: If DB is empty, uses -1 to allow all Excel rows to pass without extra logic.
 * 3. Integer Comparison: Pre-calculates numeric timestamps for ultra-fast filtering.
 * 4. Bulk Processing: Collects aggregation data during the same pass as the unique check.
 */
export async function processAdjustments(
  staFile: File | null,
  summary_id: number,
  ...
  try {
    ...
    // SENTINEL FALLBACK: If no row is found, -1 ensures all valid Excel dates (always > 0) pass the filter.
    dbLatestTime = latestDateStr ? new Date(latestDateStr).getTime() : -1;
    console.log(`[Step 1] Latest DB Date: ${latestDateStr || 'NONE (Processing all rows)'}`);
  } catch (error: any) {
    console.error(`[Step 1] DB Date Fetch failed: ${error.message}`);
  ...
  try {
    ...
    // Applying Timezone Fix immediately after parse
    allRows.forEach((row: any) => {
      const val = row['SA Date'];
      if (val instanceof Date) val.setTime(val.getTime() + (12 * 60 * 60 * 1000));
    });
  ...
  // --- 4. Filter Rows (Optimized Numeric Comparison) ---
  const dateFilteredRows = allRows.filter((row: any) => {
    const saDate = row['SA Date'] as unknown as Date;
    // Primitive number comparison is significantly faster than object/string comparison
    return (
      saDate instanceof Date && 
      !isNaN(saDate.getTime()) && 
      saDate.getTime() > dbLatestTime
    );
  });
  ...
  for (const row of dateFilteredRows) {
    ...
    const existing = await query<RowDataPacket[]>({
        query: `SELECT id FROM stock_adjustment WHERE batch_number = ? AND grade = ? AND adjusted_quantity = ? AND adjustment_date = ?`,
        values: [batchNo, grade, qty, saDate.toISOString().split('T')[0]],
    });
    ...
    if (existing && existing.length === 0) {
      ...
      insertPromises.push(query({
        query: `INSERT INTO stock_adjustment (summary_id, adjustment_date, grade, adjusted_quantity, strategy, batch_number, reason) VALUES (?, ?, ?, ?, ?, ?, ?)`,
        values: [summary_id, saDate.toISOString().split('T')[0], grade, qty, strategy.toUpperCase(), batchNo, reason]
      }));
    }

    // Accumulate results for the return object
    grouped[grade] = (grouped[grade] || 0) + qty;
    totalAdjustment += qty;
  }

  // Await all database inserts in parallel
  if (insertPromises.length > 0) await Promise.all(insertPromises);

  console.log(`--- processAdjustments END: Processed ${dateFilteredRows.length} rows ---`);
  return { totalAdjustment, groupedData: Object.entries(grouped) };
}
...
export async function update_daily_summary(
    ...
    console.log(`Updating summary ${summary_id} with WIP: ${wip_qty}, B4P: ${b4p_qty}`);
