...
export async function POST(request: NextRequest) {
    try {
        ...
        // Updated type to include 'reason'
        const failedBatchNumbers: ({batch: string | number, grade:string | null, qty:number|null, reason: string})[] = [];

        console.log(`[API] Processing ${totalReceived} records...`);

        for (const record of records) {
            const missingFields: string[] = [];

            // Check specific fields to identify EXACTLY what is missing
            if (record.cost_usd_50 === null || record.cost_usd_50 === undefined || record.cost_usd_50 === '') {
                missingFields.push('cost_usd_50');
            }
            if (record.hedge_usc_lb === null || record.hedge_usc_lb === undefined || record.hedge_usc_lb === '') {
                missingFields.push('hedge_usc_lb');
            }
            if (record.diff_usc_lb === null || record.diff_usc_lb === undefined || record.diff_usc_lb === '') {
                missingFields.push('diff_usc_lb');
            }
            
            if (missingFields.length > 0) {
                const reasonMsg = `Missing financials: [${missingFields.join(', ')}]`;
                
                // LOGGING: Print exactly why this specific batch failed
                console.warn(`⚠️ SKIP Batch: ${record.batch_number} | Grade: ${record.grade} | ${reasonMsg}`);

                // Collect the batch_number of the failed insertion with the reason
                if (record.batch_number) {
                    failedBatchNumbers.push({
                        batch: record.batch_number, 
                        grade: String(record.grade), 
                        qty: Number(record.weight),
                        reason: reasonMsg
                    });
                }
            } else {
        ...
        // Log the full list of failures for debugging
        if (failedBatchNumbers.length > 0) {
            console.log("--- FAILED BATCHES DETAILS ---");
            console.log(JSON.stringify(failedBatchNumbers, null, 2));
        }
